...
export function findOrCreateDevice(pusher: Pushbullet, nickname: string): Promise<Device> {
  return pusher.devices().then((result): Promise<Device> | Device => {
    const appDevice = result.devices.find((device: Device): boolean => device.nickname === nickname);
    if (!appDevice) return createDevice(pusher, nickname);
    return appDevice;
  });
}

export function pushToDevice(pusher: Pushbullet, deviceIden: string, title: string, message: string): Promise<Push> {
