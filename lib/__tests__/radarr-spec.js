...
import Sonarr from '../radarr';
import { searchByImdbId } from '../__mockData__/radarr.mock';
...
describe('Radarr', () => {
  ...
  describe('searchByTmdbId', () => {
    it('returns an exising movie if it already exists', () => {
      mockResponseOnce(JSON.stringify(searchByImdbId));
    ...
    it('creates a new movie if the movie can not be found', () => {
      ...
      return classInstance.searchByImdbId(NEW_TVDB_ID).then((result) => {
        expect(fetch).toHaveBeenNthCalledWith(1, `${BASE_URI}/api/movie?apikey=${API_KEY}`);
        expect(fetch).toHaveBeenNthCalledWith(
          2,
          `${BASE_URI}/api/movie/lookup/imdb?apikey=${API_KEY}&imdbId=${NEW_TVDB_ID}`,
        );
        expect(result.id).toBe(searchByImdbId.id);
      });
    });
